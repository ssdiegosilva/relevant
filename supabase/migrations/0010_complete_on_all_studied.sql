-- Session is now considered complete only when every card has been studied.
-- Previously the trigger also accepted 'saved' or 'skipped' as completion signals;
-- with the new UX (Study + Favorite buttons only), a session ends when the user
-- has actually studied every card.
...
begin
  if new.action = 'studied' then
    ...
    if not exists (
      ...
      where user_id = new.user_id and achievement_id = 'first-card'
    ) then
    ...
    if (
      ...
      where ci.user_id = new.user_id
        ...
        and ci.created_at::date = (
          now() at time zone (
            select coalesce(timezone, 'UTC') from public.profiles where id = new.user_id
          )
        )::date
    ) >= 3 then
  ...
  if new.action = 'studied' then
    ...
    if s_id is not null then
      ...
      select count(distinct ci.card_id) into studied_count
        from public.card_interactions ci
        where ci.card_id in (select id from public.cards where session_id = s_id)
          and ci.action = 'studied';
      if total > 0 and studied_count >= total then
